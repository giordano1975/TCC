...
dados_mca_completo['VALOR TOTAL ORIGINAL'] = dados_mca_completo['VALOR TOTAL']
...
dados_mca_completo['VALOR TOTAL'] = pd.qcut(dados_mca_completo['VALOR TOTAL'], 3, labels=list(['menores_valores', 'valores_medios', 'maiores_valores']))

# Calcular a média de cada segmento
media_por_segmento = dados_mca_completo.groupby('VALOR TOTAL')['VALOR TOTAL ORIGINAL'].mean()

# Contar a quantidade de valores em cada segmento
contagem_por_segmento = dados_mca_completo['VALOR TOTAL'].value_counts()

# Calcular o desvio padrão de cada segmento
desvio_por_segmento = dados_mca_completo.groupby('VALOR TOTAL')['VALOR TOTAL ORIGINAL'].std()

# Exibindo os resultados
print("Média de cada segmento:")
print(media_por_segmento)
print("\nContagem de cada segmento:")
print(contagem_por_segmento)
print("\nDesvio-padrão de cada segmento:")
print(desvio_por_segmento)

#%% Analisando as variáveis

# Categorização das variáveis

dados_mca['Valor total'] = pd.qcut(dados_mca['Valor total'], 3, labels=list(['menores_valores', 'valores_medios', 'maiores_valores']))


#%% Informações descritivas sobre as variáveis

print(dados_mca['Valor total'].value_counts())
print(dados_mca['Conformidade NB-Lei'].value_counts())

#%% Criando a tabela de contingência

tabela = pd.crosstab(dados_mca["Valor total"], dados_mca["Conformidade NB-Lei"])
print(tabela)
...
#%% Plotando o mapa perceptual - Versão melhorada (coordenadas-padrão)
...
def label_point(x, y, val, ax):
    a = pd.concat({'x': x, 'y': y, 'val': val}, axis=1)
    for i, point in a.iterrows():
        ax.text(point['x'] + 0.03, point['y'] - 0.02, point['val'], fontsize=8)  # Aumenta tamanho da fonte dos labels

# Ajustar o tamanho da figura
plt.figure(figsize=(12, 8))  # Aumenta o tamanho da figura para 12x8
...
sns.scatterplot(data=chart_df_mca, x='obs_x', y='obs_y', hue='variavel', s=60)  # Tamanho dos pontos maior (s=60)
sns.despine(top=True, right=True, left=False, bottom=False)

# Aumentar a espessura das linhas dos eixos
plt.axhline(y=0, color='lightgrey', ls='--', linewidth=1.2)  # Aumenta espessura da linha horizontal
plt.axvline(x=0, color='lightgrey', ls='--', linewidth=1.2)  # Aumenta espessura da linha vertical

# Ajustar o tamanho das marcações nos eixos
plt.tick_params(size=4, labelsize=10)  # Marcações maiores nos eixos

# Ajustar a legenda
plt.legend(bbox_to_anchor=(1.25, -0.2), fancybox=True, shadow=True, ncols=10, fontsize='8')  # Aumenta o tamanho da legenda

# Ajustar o título e os rótulos dos eixos
plt.title("Mapa Perceptual - MCA", fontsize=16)  # Título maior
plt.xlabel(f"Dim. 1: {tabela_autovalores.iloc[0, 1]} da inércia", fontsize=12)  # Label do eixo X maior
plt.ylabel(f"Dim. 2: {tabela_autovalores.iloc[1, 1]} da inércia", fontsize=12)  # Label do eixo Y maior
